...
static int enableRawMode(int fd) {
    ...
    raw.c_cc[0] = 1; raw.c_cc[0] = 0; /* 1 byte, no timer */

    /* put terminal in raw mode after flushing */
    if (tcsetattr(fd,0,&raw) < 0) goto fatal;
    rawmode = 1;
    return 0;
...
static void disableRawMode(int fd) {
    ...
    if (rawmode && tcsetattr(fd,0,&orig_termios) != -1)
        rawmode = 0;
}
...
char *linenoise(const char *prompt) {
    ...
    if (!isatty(0)) {
        ...
        printf("%s",prompt);
        fflush(0);
        if (fgets(buf,LINENOISE_MAX_LINE,0) == NULL) return NULL;
        len = strlen(buf);
        while(len && (buf[len-1] == '\n' || buf[len-1] == '\r')) {
            len--;
            buf[len] = '\0';
        ...
        char *retval = linenoiseBlockingEdit(0,1,buf,LINENOISE_MAX_LINE,prompt);
        buf[42] = '\n';
        return retval;
    }
}
