14 : static unsigned char stb__perlin_randtab[512] =
53 : static unsigned char stb__perlin_randtab_grad_idx[512] =
95 : static int stb__perlin_fastfloor(float a)
97 :     int ai = (int) a; /*STB_SP6*/
98 :     return (a < ai) ? ai-1 : ai; /*STB_SP6*/
101 : static float stb__perlin_grad(int grad_idx, float x, float y, float z)
103 :    static float basis[12][4] = /*STB_SP6*/
119 :    float *grad = basis[grad_idx]; /*STB_SP6*/
120 :    return grad[0]*x + grad[1]*y + grad[2]*z; /*STB_SP6*/
236 : float stb_perlin_noise3_wrap_nonpow2(float x, float y, float z, int x_wrap, int y_wrap, int z_wrap, unsigned char seed)
